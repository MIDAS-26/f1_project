...
export default function DriverDetail() {
  ...
  if (!frame) {
    return (
      <div className="p-2 text-zinc-400 text-center max-w-[260px]">
        Select a driver from the grid or click a car on the track to see details here
      </div>
    );
  }

  return (
    <div className="bg-zinc-800/50 backdrop-blur-sm p-4 rounded-lg border border-zinc-700 text-zinc-200 text-sm w-64">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between">
          <span>Tyres:</span>
          <span className="font-mono">
            {frame.tyre_type.toUpperCase()} · {(frame.tyre_wear * 100).toFixed(0)}% worn
          </span>
        </div>
        <div className="flex justify-between">
          <span>DRS:</span>
